    ...
    <div class=" px-4 py-2 flex justify-center">
        <div
            ...
            <div class="flex justify-start px-4 py-2 w-full">
                <a href="{{ route('Alternatif.index') }}" class="btn btn-error btn-md">Tutup!</a>
            </div>
            <table class="table w-full table-bordered rounded-sm table-zebra">
                <tr>
                ...
                <tr>
                    <td class="border border-black !bg-info text-white">Kecamatan</td>
                    <td class="border border-black ">{{ $alternatif->kecamatan_id }}</td>
                </tr>
                <tr>
            ...
            <table class="table w-full table-bordered rounded-sm table-zebra">
                @foreach ($alternatif->subalternatif as $item)
                    <tr>
                        <td class="border border-black !bg-info text-white w-1/4">{{ $item->kriteria != null ? $item->kriteria->name : '' }}</td>
                        <td class="border border-black w-1/2">{{ $item->sub_kriteria }}</td>
                    </tr>
                @endforeach
            </table>
